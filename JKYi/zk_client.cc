...
void ZKClient::OnWatcher(zhandle_t* zk,int type,int stat,const char * path,
                                                             void * watcherCtx){
...
bool ZKClient::reconnect(){
    ...
    m_handle = zookeeper_init(m_hosts.c_str(),&ZKClient::OnWatcher,m_recvTimeout,
                                 nullptr,this,0);
...
bool ZKClient::init(const std::string& hosts,int recv_timeout,watcher_callback cb
    ...
    m_handle = zookeeper_init(hosts.c_str(),&ZKClient::OnWatcher,m_recvTimeout,
                                 nullptr,this,0);
    return m_handle != nullptr;
}
...
//    return rt;
//}
int32_t ZKClient::create(const std::string& path,const std::string& val,
                            std::string& new_path,const struct ACL_vector * acl,
                            int flags){
...
int32_t ZKClient::get(const std::string& path,std::string& val,bool watch,
                         Stat * stat){
    int len = val.size();
    auto rt = zoo_get(m_handle,path.c_str(),watch,&val[0],&len,stat);
    if(rt == ZOK){
        val.resize(len);
    }
    return rt;
...
//    return get(ZOO_CONFIG_NONE,val,watch,stat);
//}
int32_t ZKClient::set(const std::string& path,const std::string& val,int version,
                        Stat * stat){
    return zoo_set2(m_handle,path.c_str(),val.c_str(),val.size(),version,stat);
}
int32_t ZKClient::getChildren(const std::string& path,std::vector<std::string>&val,
