...
std::pair<HttpResult::ptr,WSConnection::ptr> WSConnection::Create(Uri::ptr uri,
    ...
    if(!addr){
        return std::make_pair(std::make_shared<HttpResult>(
                      (int)HttpResult::Error::INVALID_HOST,nullptr,"invalid host:" + 
                       uri->getHost()),nullptr);
    }
    ...
    if(rt < 0){
        return std::make_pair(std::make_shared<HttpResult>(
                      ...
                      "send request error" ),nullptr);
    }
    auto rsp = conn->recvResponse();
    if(!rsp){
       return std::make_pair(std::make_shared<HttpResult>(
                      (int)HttpResult::Error::TIMEOUT,nullptr,
                      "recv response timeout"),nullptr);
    }
    if(rsp->getStatus() != HttpStatus::SWITCHING_PROTOCOLS){
       return std::make_pair(std::make_shared<HttpResult>(
                      50,nullptr,
                      "not wensocket server"),nullptr);
    }
    return std::make_pair(std::make_shared<HttpResult>(
                      (int)HttpResult::Error::OK,rsp,
                      "ok"),conn);
}
