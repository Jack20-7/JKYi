...
static ConfigVar<std::map<std::string,std::map<std::string,std::string>>>::ptr 
                        g_mysql_dbs = JKYi::Config::Lookup("mysql.dbs",std::map<std::string,std::map<std::string,std::string>>(),"mysql dbs");
...
bool time_to_mysql_time(const time_t& ts,MYSQL_TIME& mt){
    struct tm tm;
    localtime_r(&ts,&tm);
...
static MYSQL* mysql_init(std::map<std::string,std::string>& params,
    ...
    if(mysql == nullptr){
        JKYI_LOG_ERROR(g_logger) << "mysql_init error";
        return nullptr;
    }
    if(timeout > 0){
    ...
    int port = JKYi::GetParamValue(params,"port",0);
    std::string host = JKYi::GetParamValue<std::string>(params,"host");
    std::string user = JKYi::GetParamValue<std::string>(params,"user");
    std::string passwd = JKYi::GetParamValue<std::string>(params,"passwd");
    std::string dbname = JKYi::GetParamValue<std::string>(params,"dbname");
    if(mysql_real_connect(mysql,host.c_str(),user.c_str(),passwd.c_str(),dbname.c_str(),
                             port,NULL,0) == nullptr){
        JKYI_LOG_ERROR(g_logger) << "mysql_real_connect" << host
                                 <<" ," << port << ", " << dbname
                                 <<" error,errstr = " << mysql_error(mysql);
        mysql_close(mysql);
        return nullptr;
    }
    return mysql;
...
bool MySQL::connect(){
    ...
    m_hasError = false;
    m_poolSize = JKYi::GetParamValue(m_params,"pool",5);
    m_mysql.reset(m,mysql_close);
    return true;
}
JKYi::IStmt::ptr MySQL::prepare(const std::string& sql){
...
int MySQL::execute(const char * format,va_list ap){
    m_cmd = JKYi::StringUtil::Formatv(format,ap);
    int r = ::mysql_query(m_mysql.get(),m_cmd.c_str());
    if(r){
        JKYI_LOG_ERROR(g_logger) << "cmd = " << cmd()
                                 << ", error = " << getErrStr();
        m_hasError = true;
    }else{
        m_hasError = false;
...
std::shared_ptr<MySQL> MySQL::getMySQL(){
    return MySQL::ptr(this,JKYi::nop<MySQL>);
}
...
uint64_t MySQL::getAffectedRows(){
    ...
    //返回前一次MYSQL操作所影响的记录的行数
    return mysql_affected_rows(m_mysql.get());
}
static MYSQL_RES* my_mysql_query(MYSQL* mysql,const char * sql){
    if(mysql == nullptr){
        JKYI_LOG_ERROR(g_logger) << "mysql_query mysql is nullptr";
        return nullptr;
    }
    if(sql == nullptr){
        JKYI_LOG_ERROR(g_logger) << "mysql_query sql is nullptr";
        return nullptr;
    }
    if(::mysql_query(mysql,sql)){
       JKYI_LOG_ERROR(g_logger) << "mysql_query( " << sql << " )error: " 
                                << mysql_error(mysql);
       return nullptr;
    }
    MYSQL_RES * res = mysql_store_result(mysql);
    if(res == nullptr){
       JKYI_LOG_ERROR(g_logger) << "mysql_store_result() error : " <<mysql_error(mysql);
    }
    return res;
}
MySQLStmt::ptr MySQLStmt::Create(MySQL::ptr db,const std::string& stmt){
   ...
   if(mysql_stmt_prepare(st,stmt.c_str(),stmt.size())){
      JKYI_LOG_ERROR(g_logger) << "stmt=" <<stmt
...
int MySQLStmt::bindTime(int idx,const time_t& value){
    return bindString(idx,JKYi::Time2Str(value));
}
...
std::string MySQLRes::getColumnName(int idx){
    auto field = mysql_fetch_field_direct(m_data.get(),idx);
    return field->name;
}
...
int64_t MySQLRes::getInt64(int idx){
    return JKYi::TypeUtil::Atoi(m_cur[idx]);
}
...
double MySQLRes::getDouble(int idx){
    return JKYi::TypeUtil::Atof(m_cur[idx]);
}
...
time_t MySQLRes::getTime(int idx){
    ...
    return JKYi::Str2Time(m_cur[idx]);
}
bool MySQLRes::next(){
...
#undef XX
   ...
   //绑定结果缓冲区到m_binds,也就是相当于绑定到了m_datas
   ...
   //执行
   stmt->execute();
   //将执行结果全部缓存到本地，这样下一次就可以通过mysql_stmt_fetch去拿取
...
ISQLData::ptr MySQL::query(const std::string& sql){
    m_cmd = sql;
    MYSQL_RES* res = my_mysql_query(m_mysql.get(),sql.c_str());
    if(!res){
        m_hasError = true;
        return nullptr;
    }
...
int MySQLTransaction::execute(const char * format,va_list ap){
    if(m_isFinished){
        JKYI_LOG_ERROR(g_logger) << "transaction is finished,format = " << format;
        return -1;
    }
...
int MySQLTransaction::execute(const std::string& sql){
    if(m_isFinished){
        JKYI_LOG_ERROR(g_logger) << "transaciont is finished,sql = " << sql;
        return -1;
    }
    ...
    if(it != m_conns.end()){
        if(!it->second.empty()){
           ...
           if(rt->ping()){
               ...
               JKYI_LOG_ERROR(g_logger) << "reconnect " << name << " fail";
               return nullptr;
           }
        }
    }
    //如果上面的走不通的话，那么就个根据配置文件重新创建一个
...
void MySQLManager::registerMySQL(const std::string& name,const std::map<std::string,
                                             std::string>& params){
    MutexType::Lock lock(m_mutex);
...
int MySQLManager::execute(const std::string& name,const char * format,va_list ap){
    auto conn = get(name);
    if(!conn){
        JKYI_LOG_ERROR(g_logger) << "MySQLManager::execute,get(" << name << ") fail,"
                                    "format = " << format;
        return -1;
    }
...
ISQLData::ptr MySQLManager::query(const std::string& name,const char * format,
    ...
    auto conn = get(name);
    if(!conn){
        JKYI_LOG_ERROR(g_logger) << "MySQLManager::query ,get( " << name << ") fail,"
                                    " format = " << format; 
        return nullptr;
    }
...
MySQLTransaction::ptr MySQLManager::openTransaction(const std::string& name,
    ...
    auto conn = get(name);
    if(!conn){
        JKYI_LOG_ERROR(g_logger) << "MySQLManager::openTransaction,get(" << name 
                                 << " ) fail";
        return nullptr;
    }
...
ISQLData::ptr MySQLUtil::Query(const std::string& name,const char * format,va_list ap){
    auto conn = MySQLMgr::GetInstance()->get(name);
    if(!conn){
        return nullptr;
    }
    return conn->query(format,ap);
...
ISQLData::ptr MySQLUtil::TryQuery(const std::string& name,uint32_t count,
                                     const char * format,...){
...
int MySQLUtil::Excute(const std::string& name,const char * format,...){
    va_list ap;
    va_start(ap,format);
    int rt = Excute(name,format,ap);
    va_end(ap);
    return rt;
}
int MySQLUtil::Excute(const std::string& name,const char * format,va_list ap){
    auto conn = MySQLMgr::GetInstance()->get(name);
    if(!conn){
        return -1;
    }
...
int MySQLUtil::Excute(const std::string& name,const std::string& sql){
    auto conn = MySQLMgr::GetInstance()->get(name);
    if(!conn){
        return -1;
    }
...
int MySQLUtil::TryExcute(const std::string& name,uint32_t count,const char * format,
                                                                          ...){
    int rt = 0;
    for(uint32_t i = 0;i < count; ++i){
        va_list ap;
        va_start(ap,format);
        rt = Excute(name,format,ap);
        va_end(ap);
        if(!rt){
            return rt;
        }
...
int MySQLUtil::TryExcute(const std::string& name,uint32_t count,const std::string& sql){
    int rt = 0;
    for(uint32_t i = 0;i < count; ++i){
       rt = Excute(name,sql);
       if(!rt){
