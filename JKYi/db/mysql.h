#ifndef _JKYI_MYSQL_H_
#define _JKYI_MYSQL_H_
...
public:
    ...
    JKYi::IStmt::ptr prepare(const std::string& sql)override;
...
private:
    MySQLStmt(MySQL::ptr mysql,MYSQL_STMT * stmt);
private:
    MySQL::ptr m_mysql;
    MYSQL_STMT * m_stmt;
    std::vector<MYSQL_BIND> m_binds;
...
public:
    ...
    void registerMySQL(const std::string& name,const std::map<std::string,
                                                                    std::string>& params);
    ...
    //这里用到了万能引用，参数为模参数的右值引用类型的函数能够接受类型的实参
    //然后通过forward来实现参数的完美转发
    //template<class... Args>
...
public:
    ...
    static int Excute(const std::string& name,const char * format,...);
    static int Excute(const std::string& name,const char * format,va_list ap);
    static int Excute(const std::string& name,const std::string& sql);

    static int TryExcute(const std::string& name,uint32_t count,const char * format,...);
    static int TryExcute(const std::string& name,uint32_t count,const std::string& sql);
};

typedef JKYi::Singleton<MySQLManager> MySQLMgr;

//下面这些函数用来实现一次绑定多个参数
//template<class... Args>
//int MySQL::execStmt(const char * stmt,Args&&... args){
//    JKYi::MySQLStmt::ptr st = JKYi::MySQLStmt::Create(shared_from_this(),stmt);
//    if(!st){
//        return -1;
//    }
//    int idx = 0;
...
//}
//template<class... Args>
//ISQLData::ptr MySQLManager::queryStmt(const std::string& name,
